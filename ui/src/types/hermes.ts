...
export interface LivingColorConfig {
  agent?: {
...
export type LivingColorConfigRecord = Record<string, unknown>
...
export interface ModelOptionProvider {
  ...
  /** Managed only: whether the current account is on the free tier. */
  free_tier?: boolean
  /** Managed only: paid models a free-tier user cannot select (shown disabled). */
...
export interface ToolProvider {
   ...
   *  CLI tool active-provider detection). */
  is_active: boolean
}
...
export interface ModelAssignmentResponse {
  ...
  base_url?: string
  /** Toolset keys auto-routed through the managed tool gateway as a result of
   *  switching the main provider to Managed. Empty unless provider === 'nous'
