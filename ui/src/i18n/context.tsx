...
import { getLivingColorConfigRecord, type LivingColorConfigRecord, saveLivingColorConfig } from '@/livingcolor'
...
export interface I18nConfigClient {
  getConfig: () => Promise<LivingColorConfigRecord>
  saveConfig: (config: LivingColorConfigRecord) => Promise<{ ok: boolean }>
}

const defaultConfigClient: I18nConfigClient = {
  getConfig: () => {
    if (typeof window === 'undefined' || !window.livingColorDesktop?.api) {
      return Promise.resolve({})
    }

    return getLivingColorConfigRecord()
  },
  saveConfig: config => {
    if (typeof window === 'undefined' || !window.livingColorDesktop?.api) {
    ...
    return saveLivingColorConfig(config)
  }
}
...
export function getConfigDisplayLanguage(config: LivingColorConfigRecord): unknown {
  return isRecord(config.display) ? config.display.language : undefined
}

export function withConfigDisplayLanguage(config: LivingColorConfigRecord, locale: Locale): LivingColorConfigRecord {
