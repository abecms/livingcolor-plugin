...
> **Hermes Bible flow** — Guides · Beginner · ~6 min read  
> **Summary:** The Hermes community posts a lot about SOUL.md, Telegram bots, and overnight cron jobs — but relatively little about the dashboard as a daily tool. For me it's the most powerful surface Hermes ships. This is what I actually use it for, every day.  
> **Tags:** `dashboard`, `daily-workflow`, `mcp`, `skills`, `sessions`, `operations`
>
> **Author:** Tamsi Besson  
> **Contacts:** X [@tamsi_besson](https://x.com/tamsi_besson) · GitHub [abecms/livingcolor-plugin](https://github.com/abecms/livingcolor-plugin)

> **Screenshots:** captured from **Hermes Agent v0.17.0** (`hermes dashboard` on macOS). Before publishing publicly, redact hostnames or project names visible in your own captures if needed.
...
| Surface | Community attention |
|---------|---------------------|
| SOUL.md / personality | Very high |
| Telegram / gateway setup | Very high |
| Cron / overnight automation | High |
| Multi-agent / Kanban | Growing fast |
| **Web dashboard as daily ops tool** | Much lower |
...
To be fair: Hermes Desktop shares the same backend and covers overlapping ground (Skills, Cron, Channels, Profiles). I use Desktop for chat and the browser dashboard for ops — they're complementary, not competing. The official docs have a full [Web Dashboard](https://hermes-agent.nousresearch.com/docs/user-guide/features/web-dashboard) page. What's missing is the *practitioner angle* — someone saying "I live here, here's my actual routine."
...
![Hermes dashboard — Sessions is the default home; the sidebar lists every ops surface, with gateway status pinned at the bottom](hermes-dashboard-flow/sessions.png)
...
![MCP tab — configured servers with enable/disable and test; catalog below for one-click installs](hermes-dashboard-flow/mcp.png)
...
![Skills tab — search, category filters, per-skill toggles, and hub install in one place](hermes-dashboard-flow/skills.png)
...
![Cron tab — create jobs at the top; list below shows schedule, delivery target, pause/resume, and trigger-now](hermes-dashboard-flow/cron.png)
...
![Analytics tab — enable via Config → dashboard.show_token_analytics](hermes-dashboard-flow/analytics.png)

### When something breaks: Logs + System

**Logs** — filter by gateway/agent/errors, tail live, find the line.

![Logs tab — file selector, level/component filters, live tail](hermes-dashboard-flow/logs.png)

**System** — host stats, gateway start/stop/restart, memory provider, credential pools, `doctor`, backup/restore, skill curator.

![System tab — host metrics, gateway controls, curator, and one-click maintenance actions](hermes-dashboard-flow/system.png)
...
| What I'm doing | Where I go |
|----------------|------------|
| Daily chat, file browser, native GUI | **Hermes Desktop** |
| Ops: sessions, MCP, cron, skills, logs | **Dashboard** (`hermes dashboard` or Desktop's backend) |
| Chat while already in the dashboard | **Chat tab** (embedded TUI — same as CLI, no extra terminal) |
| Scripting, piping, automation | **CLI** in a real terminal |
| Hermes reaches me on my phone | **Gateway** (Telegram, etc.) |
...
*Shared by [Tamsi Besson](https://x.com/tamsi_besson) ([@tamsi_besson](https://x.com/tamsi_besson)). The Hermes Bible is an unofficial, community-built resource and is not affiliated with Nous Research.*
