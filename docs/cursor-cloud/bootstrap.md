...
Configure these in the automation environment (names only — never commit values):
...
Hermes dashboard protects `/api/*` with `X-Hermes-Session-Token`. The token is minted at dashboard start (`HERMES_DASHBOARD_SESSION_TOKEN` or ephemeral). Cloud agents must export a fixed token before starting the dashboard so subsequent `curl`/HTTP calls can authenticate.

## MCP configuration

`setup-automation` requires Jira and GitHub MCP servers. `scripts/cloud-bootstrap.sh` writes `~/.hermes/config.yaml` entries when credentials are present:
...
Written to `~/.hermes/livingcolor/project_mapping.yaml`:
