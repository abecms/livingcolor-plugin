...
# Load credentials the agent may have written from the prompt
...
if [ "${critical_missing}" -gt 0 ]; then
  log "Preflight: ${critical_missing} blocker(s)."
  log "If credentials are in the automation prompt, run scripts/cloud_write_credentials.py first."
  log "See docs/cursor-cloud/bootstrap.md"
  exit 1
fi
